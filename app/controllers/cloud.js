    ...
    if(un) {
        sam.User.findUserWithPromise({username: un})
            .then(function (userObj) {
                if (!userObj) {
                    sam.logger.error("setUpVm failed to find user");
                    res.json(404, {msg: "User not found"});
                }
                // before we proceed, validate some user properties first
                else if (!userObj.vm_ip && sam.config.get('new_vm_defaults:images')[userObj.device_type] === undefined)
                    ...
                    return sam.cloud.setUpUser(userObj)
                    .then(function (userObj) {
                ...
                sam.logger.error("setUpVm failed:", err.message);
                res.json(500, {});
            }).done();
    }
};
...
exports.listImages = function (req, res) {
    ...
    sam.cloud.getFlavors(function (err, allflavors) {

        if (err) {
            sam.logger.error("listImages failed to get flavors:", err.message);
            ...
            sam.cloud.getImages(function (err, r) {
                ...
                if (err) {
                    sam.logger.error("listImages failed to get images:", err.message);
...
exports.listDevices = function (req,res) {
    var obj = sam.config.get("new_vm_defaults:images");
    res.json(200,obj);
};
...
exports.listVolumes = function (req, res) {
    ...
    sam.cloud.getVolumes(function (err, r) {
        if (err) {
            sam.logger.error("listVolumes failed:", err.message);
...
exports.createVolume = function (req, res) {
    ...
    sam.User.findUserWithPromise(jsonObj)
        .then(function (userObj) {
            if (!userObj) {
                sam.logger.error("createVolume failed to find user");
                res.json(404, {msg: "User not found"});
            } else {
                return sam.cloud.createVolumeForUser(userObj)
                .then(function(userObj) {
                    var u = userObj.user;
                    sam.User.update({username: u.username}, {volume_id: u.volume_id}, function (err, numberAffected, raw) {
                        if (err) {
                            sam.logger.error("createVolume failed to find user:", err.message);
                            res.json(404, {msg: "User not found"});
                        } else {
                            if (numberAffected === 1) {
                                res.json(200, {});
        ...
        .catch(function (err){
            sam.logger.error("createVolume failed:", err.message);
...
exports.assignVolume = function (req, res) {
    ...
    if( requestObj.username && requestObj.volid) {
        sam.User.findUserWithPromise({username: requestObj.username})
            .then(function (userObj) {
                if (!userObj) {
                    sam.logger.error("assignVolume failed to find user");
                    res.json(404, {msg: "User not found"});
                } else {
                    userObj.volume_id = requestObj.volid;
                    return sam.User.updateUserWithPromise(userObj)
                    .then(function (u) {
                        res.json(200, {});
                    });
                }
            }).catch(function (err) {
                sam.logger.error("assignVolume failed:", err.message);
