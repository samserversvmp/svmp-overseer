...
openstack.init = function () {
    var openstackInfo = sam.config.get('openstack');
    if (!openstackInfo) {
        sam.logger.error("Failed to initialize OpenStack: cloud_platform set to 'openstack', but no OpenStack config (openstack) found!");
        process.exit(1);
    }
...
/**
 * @param userObj {username: 'bob'}
 * @returns {adapter.deferred.promise|*|promise|Q.promise}
 */
openstack.setUpUser = function (userObj) {
    var deferred = Q.defer();
    var flvr = sam.config.get('new_vm_defaults:vmflavor');
    var user = userObj;
    var img = sam.config.get('new_vm_defaults:images')[user.device_type];
    ...
    if (user.vm_ip && user.vm_ip.length > 0) {
        ...
        openstack.createAndStartVM(user, img, flvr)
            ...
            .then(function (updatedUser){
                var update = {
                    vm_id: updatedUser.vm_id,
                    vm_ip: updatedUser.vm_ip,
                    vm_ip_id: updatedUser.vm_ip_id
                };

                sam.User.update({username: updatedUser.username}, update, function (err) {
                    if (err) {
                        deferred.reject(new Error('Failed updating user: ' + updatedUser.username));
                        openstack.destroyVM(update).done();
                    } else {
                        deferred.resolve({vm_ip: updatedUser.vm_ip});
                    }
                });
...
openstack.createAndStartVM = function (user, image, flavor) {
    var deferred = Q.defer();
    var vmObj = {
        name: "sam-user-instance_" + user.username + "-" + new Date().getTime(),
        flavor: flavor,
        image: image
    };
    openstack.computeClient.createServer(vmObj, function (err, server) {
        var poll = sam.config.get('new_vm_defaults:pollintervalforstartup');
        ...
        if (err) {
            ...
            server.setWait({status: server.STATUS.running}, poll, function (err) {
                if (err) {
                    ...
                    if (sam.config.isEnabled('new_vm_defaults:use_floating_ips')) {
...
openstack.createVolumeForUser = function (user) {
    var deferred = Q.defer();
    var nme = "sam-user-volume_" + user.username;
    var desc = "Block Storage for: " + user.username;
    var goldSize = sam.config.get('new_vm_defaults:goldsnapshotSize');
    var goldId = sam.config.get('new_vm_defaults:goldsnapshotId');
...
openstack.assignFloatingIpToVM = function (user) {
    ...
    if (sam.config.isEnabled('new_vm_defaults:use_floating_ips')) {
        ...
        var pool = sam.config.get('new_vm_defaults:floating_ip_pool'); // optional
        ...
        openstack.computeClient.allocateNewFloatingIp(pool, function (err, ip) {
            if (err) {
                ...
                sam.logger.verbose("Allocated floating IP '%s', ID '%s'", ip.ip, ip.id);
...
function deallocateFloatingIp(user) {
    ...
    openstack.computeClient.deallocateFloatingIp(user.vm_ip_id, function (err) {
        if (err) {
            ...
            sam.logger.verbose("De-allocated floating IP '%s'", user.vm_ip);
            deferred.resolve(user);
        }
    });
    return deferred.promise;
 ...
 */
openstack.destroyVM = function (obj) {
    var deferred = Q.defer();
    ...
    if (obj.vm_id && obj.vm_id.length > 0) {
        openstack.computeClient.destroyServer(obj.vm_id, function (err, id) {
            if (err) {
                deferred.reject(new Error("destroyVM failed: " + err.message));
            } else {
                sam.logger.verbose("Destroyed expired VM, ID '%s'", obj.vm_id);
...
openstack.restartVM = function (obj) {
    ...
    openstack.computeClient.rebootServer(obj.vm_id, {type: 'HARD'}, function (err) {
        if (err) {
            ...
            sam.logger.verbose("Restarted VM, ID '%s'", obj.vm_id);
            deferred.resolve(obj);
        }
    });
